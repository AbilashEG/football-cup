...
SYSTEM = """You are DEF_L, LEFT DEFENDER. Protect your goal.
...
NEVER push past x=+5. NEVER SHOOT.
...
def handler(event, context):
    try:
        ...
        logger.info("DEF_L → %s | %s", cmd["type"], cmd.get("rationale",""))
        return cmd
    except Exception as e:
        logger.error("DEF_L error: %s", e)
        return {"type":"IDLE","target_player_id":None,"target_position":None,"rationale":"DEF_L error"}
