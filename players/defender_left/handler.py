"""
players/defender_left/handler.py
=================================
Lambda function — registered in AgentCore Gateway as MCP tool "player_def_l".
Called by coach/agent.py every tick via boto3 direct invocation.
Returns ONE AgentCommand for DEF_L.
...
def_l_agent = Agent(
    ...
    system_prompt="""
You are DEF_L, the LEFT DEFENDER for your team.
...
def handler(event, context):
    ...
    try:
        ...
        command: AgentCommand = def_l_agent.structured_output(AgentCommand, prompt)
        logger.info("DEF_L → %s | %s", command.type, command.rationale)
        return command.model_dump()
    except Exception as e:
        logger.error("DEF_L handler error: %s", e)
        return {
            "type": "IDLE",
            "target_player_id": None,
            "target_position": None,
            "rationale": "DEF_L error — IDLE this tick",
        }
